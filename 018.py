#!/usr/bin/env python

# By starting at the top of the triangle below and moving to adjacent
# numbers on the row below, the maximum total from top to bottom is 23.
#    3
#   7 4
#  2 4 6
# 8 5 9 3
# That is, 3 + 7 + 4 + 9 = 23. Find the maximum total from top to
# bottom of the triangle below:

# Discussion: If we create a graph inverting the sign of each of these
# values, we can solve this with a shortest-path problem
# implementation. Dijkstra is faster but doesn't accept negative
# weights, so Bellman-Ford it is.

import euler

ST="""
75
95 64
17 47 82
18 35 87 10
20 04 82 47 65
19 01 23 75 03 34
88 02 77 73 07 63 67
99 65 04 28 06 16 70 92
41 41 26 56 83 40 80 70 33
41 48 72 33 47 32 37 16 94 29
53 71 44 65 25 43 91 52 97 51 14
70 11 33 28 77 73 17 78 39 68 17 57
91 71 52 38 17 14 91 43 58 50 27 29 48
63 66 04 68 89 53 67 30 73 16 69 87 40 31
04 62 98 27 23 09 70 98 73 93 38 53 60 04 23
"""

# Solution using graphs
@euler.benchmark
def euler18a():
    rows = [[-int(y) for y in x.split()] for x in ST.split('\n')[1:-1]]
    g = {(-1,0): {(0,0): rows[0][0]}}

    for i in range(len(rows)-1):
        for j in range(len(rows[i])):
            g[(i,j)] = {(i+1,j):  rows[i+1][j],
                        (i+1,j+1):rows[i+1][j+1]}

    i = len(rows)-1
    for j in range(len(rows[i])):
        g[(i,j)] = {}

    p, c = euler.bellman_ford(g, (-1,0))
    print min([v for k,v in c.items() if k[0] == i])*-1

# Faster solution
@euler.benchmark
def euler18b():
    values = [[int(y) for y in x.split()] for x in ST.split('\n')[1:-1]]
    values.reverse()
    for i in range(0, len(values) - 1):
        j = 0
        for k in values[i+1]:
            values[i+1][j] = max([int(k) + int(values[i][j]), int(k) + int(values[i][j+1])])
            j += 1
    print str(values[len(values)-1][0])


euler18a()
euler18b()
